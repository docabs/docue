...
export interface SFCScriptCompileOptions {
   ...
   * This is useful in e.g. @vitejs/plugin-docue where the script needs to be
...
export function compileScript(
  ...
  if (!options.id) {
    warnOnce(
      `compileScript now requires passing the \`id\` option.\n` +
        `Upgrade your vite or docue-loader version for compatibility with ` +
  ...
  if (!scriptSetup) {
    if (!script) {
      throw new Error(`[@docue/compiler-sfc] SFC contains no <script> tags.`)
    }
    // normal <script> only
  ...
  if (script && scriptLang !== scriptSetupLang) {
    throw new Error(
      `[@docue/compiler-sfc] <script> and <script setup> must have the same ` +
        `language type.`
    )
  }
  ...
  if (
    ...
    for (const key in allBindings) {
      if (
        ...
        // skip docue imports since we know they will never change
  ...
  // 12. finalize Docue helper imports
