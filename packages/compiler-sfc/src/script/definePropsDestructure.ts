...
import { genPropsAccessExp } from '@docue/shared'
...
export function processPropsDestructure(
  ...
  warnOnce(
    `This project is using reactive props destructure, which is an experimental ` +
      ...
      `To stay updated, follow the RFC at https://github.com/docuejs/rfcs/discussions/502.`
  )
...
export function transformDestructuredProps(
  ctx: ScriptCompileContext,
  docueImportAliases: Record<string, string>
) {
  if (!ctx.options.propsDestructure && !ctx.options.reactivityTransform) {
    return
  }
  ...
  walk(ast, {
    enter(node: Node, parent?: Node) {
      ...
      checkUsage(node, 'watch', docueImportAliases.watch)
      checkUsage(node, 'toRef', docueImportAliases.toRef)
      // function scopes
