...
export function resolveComponentType(
  ...
  if (isProp) {
    if (
      ...
      isProp.type === NodeTypes.ATTRIBUTE &&
      isProp.value!.content.startsWith('docue:')
    ) {
      // <button is="docue:xxx">
      // if not <component>, only is value that starts with "docue:" will be
      // treated as component by the parse phase and reach here, unless it's
      // compat mode where all is values are considered components
      tag = isProp.value!.content.slice(6)
    }
  }
...
export function buildProps(
  ...
  for (let i = 0; i < props.length; i++) {
    ...
    if (prop.type === NodeTypes.ATTRIBUTE) {
      ...
      // skip is on <component>, or is="docue:xxx"
      if (
        name === 'is' &&
        (isComponentTag(tag) ||
          (value && value.content.startsWith('docue:')) ||
          (__COMPAT__ &&
            isCompatEnabled(
              CompilerDeprecationTypes.COMPILER_IS_ON_ELEMENT,
              context
