...
import { h, createApp, Transition, ref, nextTick } from 'docuejs'
  ...
  describe('transition with v-if', () => {
    test(
      ...
      async () => {
        ...
        await page().waitForSelector('#app')
        await page().evaluate(() => {
          const { createApp, ref } = (window as any).Docue
          createApp({
            template: `
            <div id="container">
              <transition>
    ...
    test(
      ...
      async () => {
        await page().evaluate(() => {
          const { createApp, ref, h, Transition } = (window as any).Docue
          createApp({
            template: `
  ...
  describe('transition with Suspense', () => {
    ...
    test(
      ...
      async () => {
        ...
        await page().evaluate(() => {
          ...
          const { createApp, ref, h } = (window as any).Docue
          createApp({
            template: `
            <div id="container">
    ...
    test(
      ...
      async () => {
        ...
        await page().evaluate(() => {
          const { createApp, shallowRef, h } = (window as any).Docue
          const One = {
            async setup() {
    ...
    test('children mount should be called after html changes', async () => {
      ...
      await page().evaluate(() => {
        ...
        const { createApp, ref, h, onMounted } = (window as any).Docue
        createApp({
          template: `
          <div id="container">
            <transition mode="out-in">
  ...
  describe('explicit durations', () => {
    ...
    test(
      ...
      async () => {
        ...
        expect(
          `[Docue warn]: <transition> explicit duration is NaN - ` +
        ...
        expect(
          `[Docue warn]: <transition> explicit duration is not a valid number - ` +
            `got ${JSON.stringify({})}`
        ).toHaveBeenWarned()
      },
      E2E_TIMEOUT
