...
const isCorePackage = pkgName => {
  ...
  return (
    pkgName.startsWith('@docue') &&
    packages.includes(pkgName.replace(/^@docue\//, ''))
  )
}
...
async function getCIResult() {
  try {
    ...
    const res = await fetch(
      `https://api.github.com/repos/docabs/docue/actions/runs?head_sha=${sha}` +
...
async function isInSyncWithRemote() {
  try {
    ...
    const res = await fetch(
      `https://api.github.com/repos/docabs/docue/commits/${branch}?per_page=1`
    )
    const data = await res.json()
    return data.sha === sha
  } catch (e) {
